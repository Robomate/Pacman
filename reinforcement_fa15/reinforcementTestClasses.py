...
class ValueIterationTest(testClasses.TestCase):
    # This class is unused and no long works, because ValueIterationAgent (batch update) was removed.
...
class PolicyIterationTest(ValueIterationTest):
    def __init__(self, question, testDict):
        ...
        self.discount = float(testDict['discount'])
        self.grid = gridworld.Gridworld(parseGrid(testDict['grid']))
        iterations = int(testDict['policyIterations'])
        if 'noise' in testDict: self.grid.setNoise(float(testDict['noise']))
        if 'livingReward' in testDict: self.grid.setLivingReward(float(testDict['livingReward']))
        maxPreIterations = 10
        self.numsIterationsForDisplay = range(min(iterations, maxPreIterations))
    ...
    def runAgent(self, moduleDict, numIterations):
        agent = moduleDict['policyIterationAgents'].PolicyIterationAgent(self.grid, discount=self.discount, iterations=numIterations)
        states = self.grid.getStates()
        actions = list(reduce(lambda a, b: set(a).union(b), [self.grid.getPossibleActions(state) for state in states]))
        values = {}
        qValues = {}
...
class Question5Test(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        if hashedSolution == '257635403d20faa38961f86bf7ab0945be77e48c':
            return self.testPass(grades)
        else:
...
class Question9Test(testClasses.TestCase):
    def __init__(self, question, testDict):
        super(Question9Test, self).__init__(question, testDict)

    def execute(self, grades, moduleDict, solutionDict):
        studentSolution = moduleDict['analysis'].question9()
        studentSolution = str(studentSolution).strip().lower()
        hashedSolution = sha1(studentSolution).hexdigest()
...
def computePolicy(moduleDict, grid, discount):
    valueIterator = moduleDict['valueIterationAgents'].AsynchronousValueIterationAgent(grid, discount=discount)
    policy = {}
