...
class CornersProblem(search.SearchProblem):
    ...
    def getResult(self, state, action):
        ...
        if (nextx, nexty) == self.corners[0]:
            nextcorner = (1, state[1][1], state[1][2], state[1][3])
        elif (nextx, nexty) == self.corners[1]:
            nextcorner = (state[1][0], 1, state[1][2], state[1][3])
        elif (nextx, nexty) == self.corners[2]:
            nextcorner = (state[1][0], state[1][1], 1, state[1][3])
        elif (nextx, nexty) == self.corners[3]:
            nextcorner = (state[1][0], state[1][1], state[1][2], 1)
        else:
            nextcorner = state[1]
        return ((nextx, nexty), nextcorner)
...
def cornersHeuristic(state, problem):
    ...
    for i in range(4):
        if not state[1][i]: 
            tmp = max(tmp, util.manhattanDistance(state[0], corners[i]))
            flag = 0
    if not flag:
        return tmp
...
def foodHeuristic(state, problem):
    ...
    for i in foodGrid.asList():
        if foodGrid[i[0]][i[1]]:
            tmp = max(tmp, util.manhattanDistance(position, i))
            flag = 0
    if not flag:
        return tmp
    return 0
