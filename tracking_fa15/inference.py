...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        number = random.random() * self.total()
        keys, values = self.keys(), self.values()
        distribution = values[0]
        l = len(self)
        for i in range(l):
            if number < distribution:
                return keys[i]
            elif i != l - 1:
                distribution += values[i + 1]
...
class ExactInference(InferenceModule):
    ...
    def predict(self, gameState):
        ...
        for p in self.allPositions:
            ...
            for np in newPosDist.keys():
                newBeliefs[np] += self.beliefs[p] * newPosDist[np]
...
class ParticleFilter(InferenceModule):
    ...
    def update(self, observation, gameState):
        ...
        for p in self.particles:
            weight[p] += self.getObservationProb(observation, pacmanPosition, p, jailPosition)
        if weight.total() != 0:
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(self.numParticles):
            p = ghostPositions[i % len(ghostPositions)]
            self.particles.append(p)
    ...
    def update(self, observation, gameState):
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                jointProb *= self.getObservationProb(observation[i], pacmanPosition, p[i], jailPosition)
            weight[p] += jointProb
        if weight.total() != 0:
            self.particles = [weight.sample() for i in range(self.numParticles)]
        else:
